...
export default function OpinionsSection() {
  const opinions = getArticlesByCategory("opinions").slice(0, 3);
  const stories = getArticlesByCategory("stories").slice(0, 3);
  const allItems = [...opinions, ...stories].slice(0, 4);
  ...
  return (
    <section className="py-16 md:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <SectionHeader
          title="أقلام الناس"
          subtitle="مقالات من قلب المجتمع، بأقلام أبنائه"
          action={
            <Link
              href="/opinions"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {allItems.map((article) => (
            <Link key={article.id} href={`/articles/${article.slug}`} className="group block">
              <div className="bg-ivory rounded-2xl p-5 flex gap-4 hover:shadow-card-hover transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="flex-1 min-w-0">
                  <Badge variant="gold" className="mb-2 text-[10px]">
                    {article.category === "opinions" ? "قلم مشارك" : "قصة وكفاح"}
                  </Badge>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-text-muted font-tajawal">{article.author.name}</span>
                    </div>
